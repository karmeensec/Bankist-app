...
const displayMovements = function (acc, sort = false) {
  ...
  const sortingMovs = sort ? acc.movements.slice().sort((a, b)=> a-b) : acc.movements;



  sortingMovs.forEach( function(mov, i) {
    ...
    containerMovements.insertAdjacentHTML('afterbegin', htmlTags);
...
const createUsername = function(accs) {
  ...
  accs.forEach(function (acc) {
    ...
    acc.username = acc.owner.toLowerCase().split(' ').map(function (letter, i) {
...
const startLogOutTimer = function() {
  ...
  const tick = function() {
    ...
    // each call print the remaining time ti UI

    labelTimer.textContent = `${min} : ${sec}`;

    // expired, stop timer and log out

    if (time === 0 ) {

      clearInterval(timer);
      
      labelWelcome.textContent = 'You are now logged out. Log in to get Started!';
...
btnTransfer.addEventListener('click', function(e) {
  ...
  if (amount > 0 && currentAccount.balance >= amount && receiveAccount && receiveAccount?.username !== currentAccount.username) { 
